...
def pywps_service():
    ...
    #try:
        ...
        # Línea incorporada para facilitar la implementación del código de Mario Alberto
        # a el servicio PyWPS
        __name__ = '__main__'     
        if __name__ == '__main__':
            ...
            meteoname    = request.form.get("meteoname")
            ...
            undef          = request.form.get("undef")
            swthr          = request.form.get("swthr")
            ...
            # mad parameters            
            nscan    = request.form.get("nscan")
            nfill    = request.form.get("nfill")
            z        = request.form.get("z")
            deriv    = request.form.get("deriv")
            # ustar parameters            
            ustarmin        = request.form.get("ustarmin")
            nboot           = request.form.get("nboot")
            plateaucrit     = request.form.get("plateaucrit")
            ...
            sw_dev   = request.form.get("sw_dev")
            ta_dev   = request.form.get("ta_dev")
            vpd_dev  = request.form.get("vpd_dev")
            longgap  = request.form.get("longgap")
            # partitioning parameters            
            nogppnight  = request.form.get("nogppnight")
            # daily gpp computation parameters            
            carbonfluxlimit       = request.form.get("carbonfluxlimit")             #Maximun daily GPP (Allows to remove remaining outliers)
            respirationlimit      = request.form.get("respirationlimit")            #Minimun daily respiration (Allows to remove remaining outliers)
            rolling_window_gpp    = request.form.get("rolling_window_gpp")
            rolling_center_gpp    = request.form.get("rolling_center_gpp")
            rolling_min_periods   = request.form.get("rolling_min_periods")
            # climatological footprint parameters            
            altitude                         = request.form.get("altitude")
            latitude                         = request.form.get("latitude")
            longitude                        = request.form.get("longitude")
            canopy_height                    = request.form.get("canopy_height")
            displacement_height              = request.form.get("displacement_height")
            roughness_lenght                 = request.form.get("roughness_lenght")
            instrument_height_anenometer     = request.form.get("instrument_height_anenometer")
            instrument_height_gas_analyzer   = request.form.get("instrument_height_gas_analyzer")                        
            ...
            boundary_layer_height           = request.form.get("boundary_layer_height") 
            # vegetation indices parameters             
            max_cloud_coverage               = request.form.get("max_cloud_coverage")  #Default: No filter, all images available.
            crs                              = request.form.get("crs")                 #EPSG:4326
            ndviMask                         = request.form.get("ndviMask")            #Default: No mask
            mndviMask                        = request.form.get("mndviMask")           #Default: No mask        
            # environmental vegetation parameters            
            rolling_window_ev_meteo          = request.form.get("rolling_window_ev_meteo") 
            rolling_window_ev_meteo_sat      = request.form.get("rolling_window_ev_meteo_sat") 
            rolling_window_gpp_MODIS         = request.form.get("rolling_window_gpp_MODIS") 
            precipitation_data               = request.form.get("precipitation_data").split(',')
            scale_satellite_data             = request.form.get("scale_satellite_data")          
            # mapping parameters            
            feature_collection               = request.form.get("feature_collection")
            ecosystem_extension              = request.form.get("ecosystem_extension")
            number_clusters                  = request.form.get("number_clusters")
            training_scale                   = request.form.get("training_scale")
            training_dataset                 = request.form.get("training_dataset")
            scale_getRegion                  = request.form.get("scale_getRegion")
            vector_scale                     = request.form.get("vector_scale")            
        ...
        print("Línea 422 tipo de variable hout y contenido")
        print(type(df[hout[0]]))
        print(df[hout[0]])
        print("Línea 425 tipo de variable swthr y contenido")
        print(type(swthr))
        print(swthr)
        ...
        i_resultfile=resultfile()
        i_resultfile.id_GPPD=ID
        ...
        if daily_gpp:                                                                                       
            ...
            daily_file = ID + "_2_GPP_daily.csv"
            i_resultfile=resultfile()
            i_resultfile.id_GPPD=ID
            i_resultfile.f_name=ID + "_2_GPP_daily.csv"
            ...
            chart.save("media/" + ID + "_3_GPP_daily.html")
            i_resultfile=resultfile(id_GPPD=ID, f_name=ID + "_3_GPP_daily.html",file=ID + "_3_GPP_daily.html",type_file=True)
            i_resultfile.save()        
